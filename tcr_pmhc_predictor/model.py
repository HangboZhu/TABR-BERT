...
class TcrPmhcClassifier(nn.Module):
    """ESM2-based TCR-pMHC classifier (d_model=1280, dense: 512->256->64->1)."""
...
class TcrPmhcClassifierBERT(nn.Module):
    """TCR-BERT-based TCR-pMHC classifier (d_model=256, dense: 200->100->50->1)."""

    def __init__(self, d_model: int = 256, tcr_maxlen: int = 30, pmhc_maxlen: int = 54):
        super().__init__()
        self.d_model = d_model
        self.tcr_maxlen = tcr_maxlen
        self.pmhc_maxlen = pmhc_maxlen
        ...
        self.dense = nn.Sequential(
            nn.Linear(pmhc_maxlen + tcr_maxlen + d_model * 2, 200),
            nn.ReLU(),
            nn.Dropout(p=0.4),
            nn.Linear(200, 100),
            nn.ReLU(),
